...
Model risk-heatmap stats over the sequence: mean=0.2947, min=0.0558, max=0.5603, std=0.1696
```
Those mean/min/max/std are the *per-frame spatial mean* of the risk heatmap, averaged/extremized across all 30 frames — i.e. how "hot" the whole field runs on average as the scene plays out. Within any single frame the field is far from flat: e.g. frame 15 (t=3.0s), the peak sits on the crossing agent's actual position rather than spread uniformly — confirming the learned field is spatially structured, not degenerate. The mean rises over the back half of the clip because the ego drives well past the spatial range most training probes were ever sampled in (see "Two rendering issues we found" below) — the field is extrapolating there, not necessarily tracking a real nearby hazard.

### Two rendering issues we found (and fixed) reviewing this GIF

1. **Fixed-viewport bug.** The ego drives at 8 m/s for the full 6s clip (~48m) but the panels' axis limits and the risk-heatmap query grid were both sized to the scene's static half-extent (30m) — so the ego (and the lead vehicle) drove off the right edge of the frame partway through, leaving only the crossing agent visible for the last several frames, with a growing blank gap where the heatmap grid no longer reached. Fixed by making the viewport (and the heatmap query range) a scrolling chase-cam window centered on the ego every frame (`viewport_bounds()` in `simulate.py`), so every agent and the full heatmap stay in frame for the whole clip.
2. **The synthetic scene is a *deliberate* worst-case conflict, and the original rendering didn't make that clear.** `generate_synthetic_scene()` places the crossing pedestrian at exactly the (x, y, t) the ego's own naive straight-line path would occupy — on purpose, so there's a genuine hazard for the risk field to detect. But Panel 1 previously just showed the ego box and the crossing-agent box coinciding with no explanation, which reads as an accident (or a modeling bug) rather than "this is the failure mode without risk-awareness." Fixed by labeling Panel 1 explicitly as the *naive, unmitigated* path with a warning annotation at the conflict moment, and by giving Panel 3 an honest before/after: a faint "ghost" box at the naive ego position plus a solid box at the model's actual risk-refined position, with the measured lateral correction (in meters) printed on the panel. That correction is real but modest in this run — typically a few centimeters to ~0.2m, not a dramatic swerve. We checked whether that was just an undertrained checkpoint (retrained up to 2500 steps: no improvement) or a probe-sampling artifact (tightened probe jitter + raised the ranking margin: also no improvement) before concluding it's a structural property of this reconstruction's loss: `risk_ranking_loss` compares the *mean* risk of agent-proximal probes against the *mean* risk of background probes, which is satisfiable by a smooth, broad elevation rather than a sharp per-point gradient at the exact conflict point — so gradient descent through the field finds a nearby local optimum quickly rather than a large avoidance maneuver. This is disclosed rather than hidden or artificially amplified.
...
`simulate.py` trains a fresh `READModel` for 400 steps (so the learned field is real, not random-init noise), then renders a synced, 3-panel top-down GIF (`assets/trajectory_simulation.gif`) across 30 timesteps of one synthetic scene, using a chase-cam viewport centered on the ego so every agent stays in frame for the whole clip:

1. **INPUT** — the raw, *unmitigated* scene: an ego vehicle on a straight in-lane reference path (explicitly labeled "naive straight-line path"), a lead vehicle driving away in-lane, and a pedestrian/cyclist crossing the lane directly into the ego's path — a deliberate worst-case conflict, flagged with a warning annotation at the moment of closest approach, not a rendering accident.
...
3. **MODEL PREDICTION** — the trained READ model's learned risk heatmap, queried densely over the scene at each timestep, concentrated along the crossing agent's actual conflict path (not uniform); a faint "ghost" box shows where the naive path would put the ego, a solid box shows the model's actual risk-refined position (via `refine_trajectory_by_risk_descent`), and the panel prints the real measured lateral correction in meters — see "Two rendering issues we found" above for why that correction is modest rather than dramatic in this reconstruction.
